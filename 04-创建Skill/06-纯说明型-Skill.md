...
更系统的触发质量评估（正例/负例/边界/基线）见 09 触发质量与 eval（暂未发布）；本篇给的是写 `description` 时就能立刻自查的四条底线。
