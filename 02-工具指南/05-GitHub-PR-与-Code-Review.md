...
- 说出 PR（Pull Request，合并请求）这条最小闭环：建分支 → 推送 → 开 PR → Review → 合并 → 删分支，并解释每一步为什么这么做。
- 用 `gh pr create` 或网页发起一个 PR，并写一份背景 / 改动 / 验证 / 关联四段齐全的描述，让 reviewer 不用猜。
- 拿一张清单从功能正确性、可读性、安全与副作用、一致性四个维度做 Code Review，并给出「在什么情况下、预期什么、实际什么」的具体意见。
- 为不同性质的 PR 选对合并策略（Squash / Merge commit / Rebase），合并后清理分支、补 `CHANGELOG.md`，不留僵尸分支和遗漏记录。

## 前置知识

- 读过 [提交到 GitHub 与 Commit 规范](./04-提交到-GitHub-与-Commit-规范.md)：会建分支、`git commit`、`git push` 把改动推上远程。这是 PR 的直接前置——PR 处理的就是「已经推上去的改动」。
- 读过 [GitHub 入门](./02-GitHub-入门.md)：知道仓库、分支、远程这些基本概念。
- 会用命令行（开终端、敲命令）；具体的 Git 命令照抄示例即可，不熟的可回 [Git 入门](./01-Git-入门.md) 查。
- 装好 GitHub CLI（`gh`）会更顺；没装也能用网页开 PR，第 2 节两种方式都给了。
...
到这里，02-工具指南里最常打交道的那条操作链——Git → GitHub → 提交规范 → PR 与 Code Review——你已经能完整走通了：建分支、规范提交、开 PR、审、合并、收尾。

工具会用之后，下一步是进入**方法论**：怎么把一个模糊的想法，一步步变成 AI 能帮你做出、你又能验收的东西。从这个角度看，[什么是 Vibe Coding？](../03-AI协作与上下文/01-什么是-Vibe-Coding.md) 是自然的衔接——它会重新定义「你」和「AI」在写代码这件事上各自的活儿。

> 这篇是 02-工具指南操作篇的收尾。想造自己的 Skill 的人，也可以直接跳到 [需求分析](../04-创建Skill/01-需求分析.md) 开始动手；其它工具篇（终端、项目目录、测试 CI、密钥安全）随时可从 [README](../README.md) 回查。
