...
SKILL.md 是 Skill 的「身份证 + 说明书」。它最关键的是开头一段被 `---` 包起来的元信息，叫 **frontmatter**（前置元数据）——这是专门给 AI 读的「名片」，不是给人看的正文：
...
知道了结构，再看 Skill 怎么「跑起来」。从你说一句话，到 AI 给出结果，中间有 5 步主线（发现 → 触发 → 加载 → 执行 → 输出），外加一条横切的「失败边界」——它不是第 6 步，而是「任何一步都可能出错」的兜底。
...
### 失败边界（Failure boundary）：横切的兜底，不是第 6 步
...
3. 从「你说一句话」到「出结果」，完整列出 5 步主线，并说说「失败边界」为什么不算第 6 步。
