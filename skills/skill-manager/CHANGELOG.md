...
### 修复
- **更新失败不再假成功**：单项更新传播 fetch、clone、复制和注册表写入错误；批量更新汇总成功/失败数量，存在失败时返回非零退出码。
- **GitHub 子目录可持续更新**：`remote_url` 只保存可克隆仓库地址，branch 与 `remote_subpath` 分字段记录；修复 `owner/repo/branch/path` 简写解析；更新时自动兼容并迁移旧 `/tree/...`、`/blob/...` 注册表记录，保留已记录的含 `/` 分支名。
- **同版本内容变化可识别**：更新判断改为远端 commit + 实际目录内容比对，不再因版本号相同漏掉上游改动。
- **失败可回滚**：候选版本先复制到目标旁的暂存目录，准备失败不触碰原 Skill；目录切换失败时自动恢复，成功后把旧版本保留为带时间戳的备份。

### 测试
- 新增 14 个本地 Git 仓库端到端回归测试，覆盖整仓、完整 URL 与简写子目录安装元数据，普通及含 `/` 分支的旧 registry 迁移、同版本更新、候选校验、无效来源、损坏 registry、两阶段复制失败、切换恢复，以及批量全部成功/部分失败。
