...
- [x] 为不同来源的 Skill 定义兼容校验策略：优先遵循 Agent Skills 标准，同时显式处理 Claude/Codex/OpenClaw 扩展字段，避免“一个本地校验器否定所有平台扩展”。→ **已完成**：`scripts/check_skills.py` 已兼容扩展字段（homepage/author/version 等不报错，见脚本 L25-26）；当前策略是「Agent Skills 标准字段必查 + 扩展字段宽容」，未做逐平台差异化规则（属增强项，非阻断）。
...
- [x] 修复 `skills/skill-template/` 对 starter 目录层级的依赖，确保复制到独立仓库后所有链接仍有效。→ **已完成（0.25.0）**：`skills/skill-template/SKILL.md` 的两个 `../../04-创建Skill/` 相对链接（指向 starter 教程）改为文字说明 + skills.sh 外链，模板复制到独立仓库不再断链。
- [x] 将模板拆成“最小教学版”和“带脚本的维护版”，清理示例中未被实际读取的 `.env.example` / 配置字段，并让许可证进入模板验收清单。→ **已完成（0.26.0，DEC-033）**：通过重新定位 03-基于模板创建为「模板总览+流程导航」解决（不必物理拆分模板）。03 的目录总览表已标明各文件必选/可选（纯说明型可删 scripts/，见 06）；许可证已进 03 的 frontmatter 全字段解析和常见错误。
- [ ] 建立一个 starter 原创、可端到端运行的标准示例 Skill，包含触发说明、正反例、真实输出、脚本测试、许可证、来源说明和维护文档。→ **暂缓（需实际创建+端到端测试）**：属较大工程，需真实创建 Skill 并在 Agent 环境跑通四类场景验证，待主线验收后启动。
- [ ] 为模板和标准示例增加真实 Agent 验证：至少验证“应该触发”“不应该触发”“正常执行”“错误输入”四类场景，而不只检查文件是否存在。→ **暂缓（依赖运行环境）**：需在 Claude Code 实际环境触发 Skill 验证，AI 写作无法替代真实触发测试。

### 多平台发现与上游同步

- [x] 根据各平台当前官方约定重新核对 Skill 发现路径，明确根 `skills/`、`.agents/skills`、`.claude/skills` 与其他兼容链接的职责，并同步 README / AGENTS / 目录树。→ **已完成（0.25.0）**：核对实际符号链接（`.claude/skills -> ../skills` 一级 + `.agents`/`.codex`/`.openclaw`/`.workbuddy` 四个二级 `-> ../.claude/skills`），AGENTS.md 补齐此前遗漏的 `.agents/skills` 记录。
...
- [ ] 加固上游同步命令：使用带时间或 SHA 的备份目录、禁止向只读 remote 推送、同步后执行 YAML/脚本/链接/测试检查，并验证依赖文档是否完整同步。→ **暂缓（需实际同步操作验证）**：`/sync-upstream` 命令已封装基本流程，加固项（带 SHA 备份、push 防护、同步后检查）需实际跑一次同步验证，不能纯文档改动。
- [x] 将存在长期本地修改的上游 Skill 明确标记为“fork/派生版”，不再按无差异镜像管理。→ **已完成（0.25.0）**：`docs/SOURCE-INDEX.md` 已将 `skills/skill-manager`（update.sh 有 190 增/40 删本地补丁，DEC-020）标记为「fork/派生版」，修正此前错误的「本地补丁：无」记录；`git-batch-commit` 保持纯镜像标记。

### 安全与文档一致性

- [x] 改写 Skill 搜索与安装流程：默认先查看清单和源码，再检查许可证、脚本、网络、凭据与 hooks；默认项目级试用，避免直接推荐全局 `-g -y` 安装。→ **已完成（0.23.0 扩写时覆盖）**：`04-创建Skill/02-搜索现成方案.md` 已含项目级试用（7 处）、许可证/脚本/hooks 检查（14 处）、`-g -y` 风险提醒（5 处）、先看源码（6 处），引用 05 第三方审查安全口径。
...
- [ ] 修复已确认失效的外部链接，并将已迁移的 Claude/OpenClaw 等文档链接更新到当前官方地址。→ **部分完成（0.25.0）**：`05-参考资料/01-官方资源汇总.md` 已给外链加「最近核对」日期，OpenClaw/ClawHub 不确定链接标「⚠️ 待核对」；`scripts/check_links.py --check-external` 外链检查开关已占位（待接入定时任务）。完整修复需逐个访问外链核实（依赖网络）。
...
- [x] 扩写 Git：分支合并、冲突解决、`restore` / `revert` / `reset` 的区别、误提交恢复和安全操作边界。→ **已完成（0.25.0）**：`02-工具指南/01-Git-入门.md` 新增 4 节（分支与合并/冲突解决 5 步/restore·revert·reset 对比表 3 行/误提交恢复与安全边界）。
- [x] 扩写 GitHub：Issue、Fork、Release、Actions、分支保护、权限与仓库可见性，补一个从 Issue 到 PR 合并的完整实战。→ **已完成（0.25.0）**：`02-工具指南/02-GitHub-入门.md` 新增 6 节（Issue/Fork/Release/Actions/分支保护/Issue→PR 实战 6 步），引用本仓库真实 `check.yml` 和分支保护配置。
...
- [x] 新增“AI 协作的安全边界”：只读调查、外部写入、凭据、提示注入、第三方内容、不确定事实和破坏性操作的确认机制。→ **已完成（0.25.0）**：`03-AI协作与上下文/12-AI协作的安全边界.md`，核心概念三等级（只读/需确认/破坏性）+ 提示注入 + 凭据 + 确认机制；边界控制不与概念入门 08 重叠（08 讲安装审查，本篇讲协作过程）。
...
- [ ] 为标准示例配套一份“从复制模板到发布”的逐步实验，让读者完成后能独立产出一个可验证 Skill。→ **暂缓（依赖标准示例 Skill 先建出）**：见 L39，标准示例 Skill 建出后再配套实验。当前 06/07/08 三层教程已含完整创建步骤可作过渡。

### `05-参考资料/`：从资料堆放升级为可维护索引

- [x] 给外部资料增加来源类型、适用平台、主题、难度、最近核对日期和当前有效状态，优先链接官方一手资料。→ **已完成（0.25.0）**：`05-参考资料/01-官方资源汇总.md` 每条加来源类型标签（📖官方文档/📦官方仓库/🌐社区/📄本地文件/🔗延伸阅读）+ 最近核对日期；OpenClaw/ClawHub 不确定链接标「⚠️ 待核对」；修正 skill-creator 机器特定路径为本仓库收录路径。
- [x] 区分“教程正文的依据”和“延伸阅读”，避免参考资料与前三个学习目录重复讲解同一内容。→ **已完成（0.25.0）**：每条标「📌 教程依据」或「🔗 延伸阅读」；教程依据标注被哪篇教程引用。
- [x] 建立资料更新机制：平台规则或链接变化时能定位受影响文章，并通过定时外链检查产生待办。→ **已完成（0.25.0）**：新增「资料更新机制」章节（外链核对周期、失效链接处理、平台规则变化定位的 grep 方法、依据vs延伸的变更影响差异）。
...
- [x] 第三批完成端到端案例和三个层级的 Skill 创建教程，再集中调整 README 导航与文章编号。→ **已完成（0.26.0）**：三层级（06/07/08）、09 触发质量 eval、03-AI协作主线（05→12 含贯穿案例+安全边界）、02-工具指南（06→10 含测试CI）、04-创建Skill 01-04 扩写（03 重新定位为模板总览+流程导航，DEC-033）均已完成。
- [ ] 每批邀请至少一名没有 Skill 开发经验的读者按顺序实操，记录卡点、完成时间和无法独立完成的步骤，再据此修订。→ **阻塞（需真人读者）**：这是内容质量的最终验收标准，AI 无法替代真实读者实操。主线 28 篇已全部落盘，待维护者安排读者走查。
- [ ] 内容完成不以“文件已创建”为准：读者必须能按文档得到预期结果，所有命令、链接和示例输出都应经过复跑验证。→ **持续进行**：每批提交均跑 `scripts/check.sh`（链接/语法/frontmatter）+ GitHub Actions CI 严格门禁；命令可跑性由写作时核对，完整复跑验证待读者实操（见上一条）。

## 后续可选增强

> 以下为可选项，**中文主线稳定 + 读者实操验收完成前不启动**（评估决策 2026-07-29）。验收后视读者反馈再决定优先级。

- [ ] ~增加不同领域的示例，但每个示例必须回答一个不同教学问题，避免只换业务名的重复样例。~ → **暂缓**：当前 todo.py 贯穿示例已覆盖 Git/PR/Skill 创建/编排/eval 多场景，待读者反馈「示例不够多样」再补。
- [ ] ~为核心概念制作小型结构图或流程图；仅在关系或流程明显比文字更易懂时使用。~ → **暂缓**：现有文字+表格已覆盖，待读者反馈「关系难懂」的篇再针对性加图。
- [ ] ~评估是否提供练习答案、FAQ、故障排查索引和文章级反馈入口。~ → **暂缓**：每篇已有自测题/验收，答案/FAQ 待读者验收收集卡点后再建。
- [ ] ~评估多语言版本；中文主线稳定前不启动全文翻译。~ → **暂缓**：中文主线刚完成，明确不启动翻译。
