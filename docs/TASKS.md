...
- [ ] 第三批完成端到端案例和三个层级的 Skill 创建教程，再集中调整 README 导航与文章编号。→ **部分推进（DEC-028）**：第三层 08 多 Skill 编排（0.20.0，PM 单线程产出）已落地，CI/分支保护（0.19.0）已闭环；剩余 09 触发质量 eval、贯穿案例、扩写 01/02/04 等。
