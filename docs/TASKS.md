...
- [ ] 第三批完成端到端案例和三个层级的 Skill 创建教程，再集中调整 README 导航与文章编号。→ **接近完成**：三层级（06/07/08）、09 触发质量 eval、03-AI协作主线（05→12 含贯穿案例+安全边界）、02-工具指南（06→10 含测试CI）、04-创建Skill 01/02/04 扩写均已完成；**仅剩 03-基于模板创建待拆最小/维护版**（有架构判断：06/07 已分别讲纯说明型/带脚本型创建，03 需重新定位为「模板使用总览」避免重叠）。
