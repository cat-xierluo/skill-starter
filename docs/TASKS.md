...
- [ ] 先制作一张“读者阶段 × 前置知识 × 学习结果 × 对应文章”的内容矩阵，再决定新增文章的编号，避免边写边重排目录。
- [ ] 在 README 增加两条入口：“零基础顺序学习”和“已有经验按问题查阅”，让教程路径与参考资料路径分开。
- [ ] 为教程统一最小结构：本篇目标、前置知识、核心概念、动手步骤、常见错误、自测题/验收、下一篇；参考资料不强制套用教程结构。
- [ ] 统一示例项目和术语，尽量让读者沿用同一个小项目贯穿 Git、Agent 协作、Skill 创建、测试和发布，减少每篇重新理解背景的成本。
- [ ] 明确跨平台范围：命令示例至少标注 macOS / Windows / Linux 的差异；平台特有行为必须注明适用版本和最近核对日期。

### `01-概念入门/`：建立完整心智模型

- [ ] 扩写“什么是 Skill”：补充 Skill 的发现、触发、加载、执行、输出和失败边界，不只停留在 App 类比。
- [ ] 新增“大模型、聊天助手、Agent、工具、MCP、Skill、Plugin 与 Workflow 的关系”，给出同一需求下各自负责什么的对照案例。
- [ ] 新增“一个 Skill 是怎样工作的”，用目录和一次执行流程解释 `SKILL.md`、scripts、references、assets 与运行环境。
- [ ] 新增“复用、改造还是新建”，提供基于匹配度、维护成本、安全风险和许可证的决策树。
- [ ] 新增“权限、凭据与信任边界”，解释为什么 Skill 能执行命令、访问文件或联网，以及安装第三方 Skill 前应检查什么。
- [ ] 新增“版本、来源与维护”，解释上游、fork、同步、语义化版本、变更记录和弃用，衔接本仓库的维护规范。
- [ ] 扩充术语表：补齐 CLI、shell、路径、环境变量、依赖、frontmatter、YAML、JSON、MCP、hook、eval、CI 等零基础高频词，并链接到对应教程。

### `02-工具指南/`：补足开始实操前的最低工具基础

- [ ] 新增“终端与命令行入门”：当前目录、绝对/相对路径、列出/复制/移动文件、引号与空格、退出码、帮助命令和安全边界。
- [ ] 新增“项目目录与文件格式入门”：隐藏文件、扩展名、Markdown、YAML、JSON、frontmatter、编码与缩进，并提供可修改的小练习。
- [ ] 新增“开发环境与依赖入门”：Python、Node.js、包管理器、`npx`、虚拟环境、版本检查和“全局安装 vs 项目安装”。
- [ ] 新增“环境变量与密钥安全”：`.env`、`.env.example`、`.gitignore`、最小权限、泄漏后的处置，以及为什么不能把 Token 写进教程或提交历史。
...
- [ ] 将现有 PR / Code Review 文档扩成可复现练习，包含审查者和作者两种视角、行内评论、修改后复审、合并策略和失败示例。
- [ ] 新增“测试、Lint、CI 是什么”，用本仓库检查脚本展示本地验证与远程门禁的区别，并说明“检查通过不等于功能真实可用”。

### `03-AI协作与上下文/`：从会聊天推进到可靠协作

- [ ] 新增“从需求到验收标准”：教读者描述目标、用户、范围、约束、输入输出、非目标和可观察的完成条件。
- [ ] 新增“Prompt、Rules、Commands、Skills、MCP 和 Hooks 怎么分工”，说明信息应该放在哪里、生命周期多长、谁会触发。
- [ ] 新增“任务拆解与迭代”：如何让 Agent 先调查、再计划、分批实现、保留回退点，并避免一次性大改。
- [ ] 新增“验证 AI 的工作”：事实核验、代码审查、测试、实际运行、截图/输出证据、失败复盘，以及如何识别假绿与幻觉。
- [ ] 新增“上下文生命周期”：会话上下文、项目上下文、压缩/遗忘、上下文污染、何时写入 TASKS/DECISIONS/CHANGELOG，以及如何控制文档膨胀。
- [ ] 新增“多 Agent 协作入门”：角色边界、任务所有权、并行条件、共享工作区冲突、交接格式和最终集成责任。
- [ ] 新增“AI 协作的安全边界”：只读调查、外部写入、凭据、提示注入、第三方内容、不确定事实和破坏性操作的确认机制。
- [ ] 新增一篇贯穿案例：从模糊想法开始，经过澄清、计划、实现、验证、文档同步和 PR，完整展示一次可靠协作。

### `04-创建Skill/`：把知识收束为可交付成果

- [ ] 扩写需求分析：加入用户画像、触发/负向触发、输入输出契约、失败处理、依赖、安全、许可证和验收标准。
- [ ] 扩写现成方案评估：加入源码审查、来源可信度、更新活跃度、许可证兼容、试运行和退出方案。
- [ ] 分别提供“纯说明型 Skill”“带脚本 Skill”“多 Skill 编排”三个由浅入深的完整教程。
- [ ] 新增触发质量与 eval 教程：设计正例、负例、边界例，记录基线并比较修改前后结果。
- [ ] 扩写调试与发布：覆盖本地发现、依赖安装、错误诊断、版本、CHANGELOG、tag/release、安装说明、升级与回滚。
