...
- **新增 `04-创建Skill/10-从复制到发布-逐步实验.md`**（T-009，DEC-042）：端到端走查教程。在干净临时目录里从模板复制出 `todo` Skill，一路走到运行、测试、打版本、发布、回滚。以 `skills/todo` 为标准答案，概念一律链接到 04/07/01-Git，不重复。**第 7 步补上 04 篇缺失的回滚实操**——故意引入假绿回归（done 总返回 0），再用 `git switch --detach v0.1.0` / `git revert` / `git reset` 三种方式回滚，回滚后契约恢复。

### Changed

- `04-创建Skill/04-调试与发布.md`：回滚定义（原只有一句话）后加互链，指向 10-逐步实验的第 7 步回滚实操。
- `docs/CONTENT-MATRIX.md`：登记 `04-创建Skill/10-从复制到发布-逐步实验`（学习结果：在干净目录从复制到回滚独立产出一个可验证 Skill）。

### Notes

- 验证：`STRICT_LINKS=1 bash scripts/check.sh` 全绿；计数准确（7 步动手步骤 / 5 条常见错误 / 5 题自测，标题数字=正文条数）。
- T-009 标记完成（决策见 DEC-042）。T-007→T-009 形成「标准示例 + 端到端实验」闭环。剩余就绪项 T-006（可复跑证据，部分依赖运行时）；阻塞项 T-008（Agent 环境）、T-010（真人读者）、待决策 T-004/T-011。
...
- **T-005 教程骨架复核收尾**（DEC-040）：范围内 11 篇较早教程全部补齐 WRITING-GUIDE 7 段骨架。4 个 subagent 并行产出后 PM 统一 review，抓到并修复一致性违规。
  - `01-概念入门/02-什么是-许可证.md`、`03-什么是-GitHub.md`：补「本篇目标 + 前置知识 + 自测题（各 4 题）」，把「风险/误区」段规范成「常见错误」（各 4 条），下一篇收敛成单一线索。
  - `02-工具指南/01-Git-入门.md`：步骤命名从阿拉伯数字 `### 1.~7.` 统一为「第一步…第七步」中文数字（对齐 02 目录 02/03/05 约定）；补「本篇目标 + 前置知识 + 自测题（5 题）」。
  - `02-工具指南/02-GitHub-入门.md`、`03-SSH-配置.md`：补「本篇目标 + 前置知识 + 自测题（各 4 题）+ 单一下一篇」。
  - `02-工具指南/04-提交到-GitHub-与-Commit-规范.md`：补「本篇目标 + 前置知识 + 自测题（4 题）」，原「建议继续读」收敛成单一下一篇。
  - `02-工具指南/05-GitHub-PR-与-Code-Review.md`：补「本篇目标 + 前置知识 + 单一下一篇」（这篇骨架最全，收尾工作量最小）。
  - `03-AI协作与上下文/01-什么是-Vibe-Coding.md`、`02-什么是-AGENTS和-CLAUDE.md`、`03-上下文工程入门.md`、`04-Rules-编写指南.md`：补「本篇目标 + 前置知识 + 自测题（各 3 题）」；02-AGENTS（补写量最大）新写「常见错误」5 条。

### Fixed

- **全仓「错误 N」机械编号清零**（WRITING-GUIDE §1.5）：03-AI协作 01-09、01-概念入门 05-09 共 9 篇教程的常见错误标题从「### 错误 N：xxx」改为纯命名「### xxx」。PM review 发现 subagent 产出的 03 目录 01-04 仍带机械编号后，顺带清理了范围外同款违规，避免留「待修」尾巴。参考资料类文章（05-目录 2 篇）按 WRITING-GUIDE §0 不强制约束，保留。

### Notes

- 验证：`STRICT_LINKS=1 STRICT_SH_SYNTAX=1 bash scripts/check.sh` 全绿；常见错误/自测题计数准确性逐篇抽查通过；各篇「下一篇」串成连贯主线（01-概念→02-工具→03-AI协作，跨目录衔接无冲突）。
- T-005 标记完成（文章级检查结果见 DEC-040）。仍阻塞：T-006 可复跑证据、T-007 标准示例、T-008 真实 Agent 验证、T-010 真人读者走查——均需运行环境/真人，AI 无法独立推进。
...
- `04-创建Skill/03-基于模板创建.md`：重新定位并扩写为「模板使用总览 + 创建流程导航」。解决与 06/07 职责重叠的架构问题（DEC-033）：03 讲模板目录结构总览表 + frontmatter 全字段 + 8 步导航（每步指向具体篇），不重复 06/07 的具体创建内容。**04-创建Skill 主线 01-09 全部对齐新风格。**

### Notes

- **TASKS 清理收尾**：L117 第三批完成、L38 模板拆分通过重新定位解决。TASKS 未完成项从 41 降至 12，剩余全部带明确阻塞原因（需真人验收/外部核实/工程实操），无模糊挂起项。

## 0.25.0 - 2026-07-29

### Changed

**TASKS.md 大清理 + C1/C3 批次推进**：

- 清理 17 个僵尸条目（Wave 1-8 已产出的文章对应条目补勾 ✅），未完成项从 41 降至 24，TASKS 反映真实状态。
- `02-工具指南/01-Git-入门.md`：新增 4 节（分支与合并 / 冲突解决 5 步 / restore·revert·reset 对比表 3 行 / 误提交恢复与安全边界）。
- `02-工具指南/02-GitHub-入门.md`：新增 6 节（Issue / Fork / Release / Actions / 分支保护 / Issue→PR 实战 6 步），引用本仓库真实 `check.yml` 和分支保护配置。
- `03-AI协作与上下文/12-AI协作的安全边界.md`（新建）：核心概念操作三等级（只读/需确认/破坏性）+ 提示注入 + 凭据 + 确认机制；边界控制不与概念入门 08 重叠。

### Fixed

- `docs/SOURCE-INDEX.md`：修正 `skills/skill-manager` 的「本地补丁：无」错误记录（实际 update.sh 有 190 增/40 删本地补丁，DEC-020），标记为「fork/派生版」。

### Notes

- L30 校验策略标记完成（check_skills.py 已兼容扩展字段）；L47 fork 标记完成；L117 第三批更新为「仅剩 03-基于模板创建」。
- 12-AI协作安全边界边界控制：grep 验证未出现 08 篇的「-g -y/安装审查」内容，与 08 篇（安装审查）主题无重叠。
